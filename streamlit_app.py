...
GOOGLE_SHEET_TAB_NAME = "result_stocks"
GCP_CREDS_FILE = Path("assets/creds.json") # Path to your service account JSON file
...
def _get_gcp_creds():
    if not GCP_CREDS_FILE.exists():
        st.error(f"Credentials file not found: {GCP_CREDS_FILE}. Please ensure it exists in your 'assets/' folder.")
        st.stop()
    try:
...
def get_google_sheet_data():
    try:
        ...
        st.error(f"Error: Worksheet tab '{GOOGLE_SHEET_TAB_NAME}' not found in '{GOOGLE_SHEET_NAME}'. Please check the tab name.")
        return {}
    except Exception as e:
        st.error(f"An unexpected error occurred while accessing Google Sheet: {e}. Please ensure:\n"
...
with tab1:
    ...
    stock_names_input = st.text_area(
        ...
        placeholder="e.g.\nNatco Pharma Ltd,\nIZMO Ltd, ACME Solar Holdings Ltd",
        height=100,
        key="text_area_input",
